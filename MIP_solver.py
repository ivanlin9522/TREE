...
def MIP_solver(trees_given, flag_given, eta_given):     
    eta=eta_given
    trees=trees_given
    flag=flag_given
    ...
    for i in total_split_variable(trees):
        X[i]=m.addVar(lb=0, ub=5, name='X'+str(i))
        for j in range(K(trees,i)):
    ...
    m.addConstr(quicksum(X[i]*X[i] for i in total_split_variable(trees)) <= eta)
    m.update()
...
def bisection(trees,flag,eta_1, eta_2,baseline):
    if eta_1 >= eta_2:
        ...
        [sol_1[iteration],x_1[iteration]]=MIP_solver(trees,flag,eta_1)
        print(x_1[iteration],"\n")
        [sol_2[iteration],x_2[iteration]]=MIP_solver(trees,flag,eta_2)
        print(x_2[iteration],"\n")
        [sol_new[iteration],x_new[iteration]]=MIP_solver(trees,flag,eta_new)
